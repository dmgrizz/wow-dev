...
module.exports = app => {
  app.get('/userProfile/:charName/:charRealm', ensureAuthenticated, async (req, res, next) => {
      var battletag = req.user.battletag;
      var charName = req.params.charName;
      var charRealm = req.params.charRealm;
      const user = await User.findOne({battletag: battletag});
        ...
        var faction = user.characters[0].faction;
        var realm = user.characters[0].realm;
        ...
        if(faction === "Horde") {
          ...
          factionPic = user.characters[0].mediaservices
        }
